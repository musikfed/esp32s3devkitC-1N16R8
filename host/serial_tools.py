...
_RUNTIME_PROBE_SCRIPT = (
    ...
    "print('__STUDIO_MACHINE__=' + u.machine);"
    "print('__STUDIO_RELEASE__=' + u.release);"
    "print('__STUDIO_UID__=' + ''.join('%02x' % b for b in uid))"
)


def _parse_runtime_probe(port: str, output: str) -> dict:
    def marker(name: str) -> str:
        match = re.search(rf"^{re.escape(name)}=(.*)$", output, re.M)
        return match.group(1).strip() if match else ""

    machine_text = marker("__STUDIO_MACHINE__")
    release = marker("__STUDIO_RELEASE__")
    uid = marker("__STUDIO_UID__")
    chip = _normalise_chip(machine_text)
    if not machine_text or chip == "unknown":
        raise SerialToolError(
            f"{port} opened, but Studio could not identify a MicroPython ESP32 REPL. "
            "Choose the runtime/REPL COM port, not the bootloader port."
        )
    mac = ":".join(uid[i:i + 2] for i in range(0, len(uid), 2)) if len(uid) >= 12 else uid
    matches = compatible_profiles(chip, None)
    return {
        "port": port,
        "chip": chip,
        "chip_text": machine_text,
        "flash_mb": None,
        "psram_mb": None,
        "mac": mac,
        "micropython": release,
...
def probe_micropython(port: str) -> dict:
    """Probe the working MicroPython REPL without invoking esptool.

    On ESP32-S3 boards the ROM/USB-Serial-JTAG flashing port and the normal
    MicroPython REPL port can be different COM devices. Runtime provisioning
    must therefore validate the selected port through mpremote itself.
    """
    try:
        output = _mpremote(port, "exec", _RUNTIME_PROBE_SCRIPT, timeout=30)
    except SerialToolError as exc:
        raise SerialToolError(
            f"Cannot enter MicroPython REPL on {port}. Select the runtime/REPL COM port "
            f"(not the esptool/bootloader port). Details: {exc}"
        ) from exc
    return _parse_runtime_probe(port, output)
...
def install_runtime(port: str, profile_id: str, ssid: str = "", password: str = "", token: str = "") -> dict[str, str]:
    ...
    probe = probe_micropython(port)
    if probe["chip"] != str(profile["chip"]).lower():
        raise SerialToolError(
            f"Selected profile is {profile['chip']}, but MicroPython on {port} reports {probe['chip_text']}."
        )
    ...
    logs: list[str] = ["== MicroPython runtime probe ==\n" + probe["log"]]
...
def probe_micropython_job(port: str, handle) -> dict:
    handle.stage("runtime_probe", progress=None, detail=f"Opening MicroPython REPL on {port}")
    try:
        output = _stream_run(
            _python_tool("mpremote", "connect", port, "exec", _RUNTIME_PROBE_SCRIPT),
            handle,
            "runtime_probe",
            30,
            False,
    ...
    result = _parse_runtime_probe(port, output)
    handle.stage_done("runtime_probe", f"{result['chip_text']} · MicroPython {result.get('micropython') or '?'}")
    return result
...
def install_runtime_job(port: str, profile_id: str, ssid: str, password: str, token: str, handle) -> dict[str, str]:
    ...
    probe = probe_micropython_job(port, handle)
    if probe.get('mac'):
        handle.set_project(
            'device:' + probe['mac'].lower().replace(':', ''),
            f"{profile['name']} · {port}",
